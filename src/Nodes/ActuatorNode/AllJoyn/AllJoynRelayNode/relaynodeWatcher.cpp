...
relaynodeWatcher::relaynodeWatcher(AllJoynBusAttachment^ busAttachment) :
...
relaynodeWatcher::~relaynodeWatcher()
{
    UnregisterFromBus();
}

void relaynodeWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "digital.pervasive.sample.relaynode" };
...
void relaynodeWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "digital.pervasive.sample.relaynode"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "digital.pervasive.sample.relaynode"),
            port);
        Added(this, args);
    }
...
void relaynodeWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<relaynodeWatcher>
        };
...
void relaynodeWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_relaynodeIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &relaynodeWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void relaynodeWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void relaynodeWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
